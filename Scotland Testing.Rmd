...
scotLULC <- rast('/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Land Cover/2022/HLCM_2022_EUNIS_LEVEL2.tif')
...
scotNP <- st_read("/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Scotland NP/National_Parks_-_Scotland.shp")
...
maxTempRast <- rast("/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Rasters/tempMaxRaster.tif")
minTempRast <- rast("/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Rasters/tempMinRaster.tif")
...
soilMoistureRast <- rast("/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Rasters/soilMoistureRast.tif")
...
write_csv(finalTable, '/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Restrictive Scenario Dataframes/finalTable.csv')
...
ggplot(data = pcaDf, aes(x = PC1, y = PC2, color = as.factor(groups))) + 
  ...
  labs(title = "Principal Component Analysis of Scotland Tree Types: PC1 and PC2", color = "Forest Type") +
...
```{r}
filepath <- '/Users/louisgoodall/Desktop/Portfolio Projects/Data/Scotland/Outputs'
...
ggplot(aucrocDf %>% filter(Model != "XGB"), aes(x = FPR, y = TPR, col = Class)) +
  geom_line(size = 1) +
...
finishedDf %>%
  ...
  arrange(Scenario, Model, Predicted_Class) %>%
  filter(Predicted_Class == "Scots pine" & Model == "NB")
```
...
probabilityDf %>%
  ...
  mutate(Birch_High = (Birch >= 0.8),
         Oak_High = (Oak >= 0.8),
         Pine_High = (`Scots pine` >= 0.8)) %>%
  group_by(x, y, Forest_Type, Model) %>%
  summarise(Birch_Count = sum(Birch_High),
            Oak_Count = sum(Oak_High),
            Pine_Count = sum(Pine_High), .groups = "drop") %>%
  mutate(Max_High = pmax(Birch_Count, Oak_Count, Pine_Count),
         Forest_Type = case_when(Birch_Count == Max_High ~ "Birch",
                                 Oak_Count == Max_High ~ "Oak",
                                 Pine_Count == Max_High ~ "Pine",
                                 TRUE ~ Forest_Type),
         Category = if_else(Max_High > 3, paste0(Forest_Type, Max_High), NA_character_)) %>%
  drop_na(Category)
