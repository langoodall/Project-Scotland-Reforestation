...
rasterNames <- c("RF Scenario Most", "RF Scenario High", "RF Scenario Moderate", "RF Scenario Least", "XGB Scenario Most", "XGB Scenario High", "XGB Scenario Moderate", "XGB Scenario Least", "ANN Scenario Most", "ANN Scenario High", "ANN Scenario Moderate", "ANN Scenario Least")

# For loop to plot the rasters
colors <- c("#38a3a5", "#ffba08", "#C75C5A")
...
ggplot(probabilityMapsAll) +
  geom_raster(aes(x = x, y = y, fill = Forest_Type)) +
  scale_fill_viridis_d() +
  theme_minimal() +
  facet_grid(Model ~ Scenario)
  # geom_sf(data = Scotland.Shp, fill = NA, color = "black", size = 0.5)



# highProbabilityMapsAll <- probabilityMapsAll %>%
#   filter(Birch >= 0.8 | Oak >= 0.8 | Pine >= 0.8)



ggplot(highProbabilityMapsAll) +
  geom_raster(aes(x = x, y = y, fill = Forest_Type)) +
  scale_fill_viridis_d() +
  theme_minimal() +
  facet_grid(Model ~ Scenario)
...
probabilityMapsAll %>%
  filter(Scenario == 4)  %>%
  mutate(Birch_High = as.numeric(Birch > 0.8),
         Oak_High = as.numeric(Oak > 0.8),
         Pine_High = as.numeric(Pine > 0.8),
         High_Prob_Types = paste(ifelse(Birch_High, "Birch", ""),
                                 ifelse(Oak_High, "Oak", ""),
                                 ifelse(Pine_High, "Pine", ""), sep = ", ") %>%
           gsub(", ,", ",", .) %>%
           trimws() %>%
           gsub("^,|,$", "", .)), High_Prob_Count = rowSums(select(., Birch_High, Oak_High, Pine_High)))
...
custom_colors <- c(
  "Birch1" = "#1b9e77", "Birch2" = "#d95f02", "Birch3" = "#7570b3",
  "Oak1" = "#dddf00", "Oak2" = "#80b918", "Oak3" = "#007f5f",
  "Pine3" = "#f8961e", "Pine2" = "#f3722c", "Pine2" = "#9c6644"
)
