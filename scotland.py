...
X_train, X_test, y_train, y_test = train_test_split(X_resampled, y_resampled, test_size = 0.2, stratify = y_resampled, random_state = 33)
...
# Use 'roc_auc_ovr' for multiclass classification
grid_search_RF = GridSearchCV(estimator = clf, param_grid = param_grid, cv = cv, scoring = 'roc_auc_ovr', n_jobs = int(os.getenv('LSB_DJOB_NUMPROC', 1)))
...
X_resampled, y_resampled = smote.fit_resample(X, y)
X_train, X_test, y_train, y_test = train_test_split(X_resampled, y_resampled, test_size = 0.2, stratify = y_resampled, random_state = 33)
xgb_model = XGBClassifier(objective = 'multi:softprob', num_class = len(np.unique(y)), n_jobs = int(os.getenv('LSB_DJOB_NUMPROC', 1)), random_state = 33)
param_grid = {
...
cv = StratifiedKFold(n_splits = 5, shuffle = True, random_state = 33)
grid_search_XGB = GridSearchCV(estimator = xgb_model, param_grid = param_grid, cv = cv,scoring = 'roc_auc_ovr', n_jobs = -1)
...
param_grid = {
    'hidden_layer_sizes': [(8,8,3), (16,8,4,3), (32,16,8,4), (32,16,8,4,3)],
...
cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=33)
grid_search_MLP = GridSearchCV(mlp, param_grid = param_grid, cv = cv, scoring = 'roc_auc_ovr', n_jobs = int(os.getenv('LSB_DJOB_NUMPROC', 1)))
grid_search_MLP.fit(X_train, y_train)
# Best model evaluation
y_pred_MLP = grid_search_MLP.best_estimator_.predict(X_test)
y_pred_proba_MLP = grid_search_MLP.best_estimator_.predict_proba(X_test)
...
cv = StratifiedKFold(n_splits = 5, shuffle = True, random_state = 33)
grid_search_SVM = GridSearchCV(estimator = svm, param_grid = param_grid, cv = cv, scoring = 'roc_auc_ovr', n_jobs = int(os.getenv('LSB_DJOB_NUMPROC', 1)))
...
cv = StratifiedKFold(n_splits = 5, shuffle = True, random_state = 33)
grid_search_NB = GridSearchCV(estimator = nb, param_grid = param_grid, cv = cv, scoring = 'roc_auc_ovr', n_jobs = int(os.getenv('LSB_DJOB_NUMPROC', 1)))
grid_search_NB.fit(X_train, y_train)
y_pred_NB = grid_search_NB.best_estimator_.predict(X_test)
y_pred_proba_NB = grid_search_NB.best_estimator_.predict_proba(X_test)
# Store AUC and ROC data
