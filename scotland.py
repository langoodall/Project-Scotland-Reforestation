...
param_grid = {
    ...
    'max_samples': [None, 0.8, 0.9],          # Subsample the training data to reduce variance
    'max_features': ['sqrt', 'log2']          # Reduce features considered at each split to prevent overfitting
}
...
param_grid = {
    ...
    'min_child_weight': [1, 3, 5],  # Reduce chance of overfitting
    'gamma': [0, 0.1, 0.2],
...
grid_search_XGB = GridSearchCV(estimator=xgb_model, param_grid=param_grid,
                               cv=cv, scoring='roc_auc_ovr')
grid_search_XGB.fit(X_train, y_train, eval_set=[(X_test, y_test)],
